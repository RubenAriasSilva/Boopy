...
namespace BoopyGame
{
    public class GameManager_DB : MonoBehaviour
    {
        ...
        public void UsuarioPredeterminado(string nombre = "")
        {
            bdActiva = false;
            if(nombre == "")
            {
                int numeroRd = UnityEngine.Random.Range(0, 2);
                bool n = (numeroRd == 1);
                if (n)
                {
                    nombre = "Bocchi";
            ...
            {
                Nombre = nombre,
                Correo = "soyuncorreo@gmail.com",
                Password = "1234",                    
                
                GatitoId = 1,
                GatoteId = 1,
                IdiomaSeleccionadoId = PlayerPrefs.GetInt("IdiomaSeleccionado", 0)
            };
